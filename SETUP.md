...
## 9. Postman collection (C-28)

Import these files into Postman:

| File | Purpose |
| ---- | ------- |
| [`unisouk-assignment.postman_collection.json`](./unisouk-assignment.postman_collection.json) | All API requests + E2E + error flows |
| [`unisouk-assignment.postman_environment.json`](./unisouk-assignment.postman_environment.json) | Pre-filled variables (URLs, seed IDs, credentials) |

1. **Import** both files (File → Import).
2. Select the **unisouk-assignment** environment (top-right).
3. Run **Auth → Login** first (sets `{{token}}`).
4. Run the **E2E Flow** folder sequentially via Collection Runner (requires order 1 in `pending` status and `worker` running).

Regenerate from source: `node scripts/build-postman-collection.mjs`

---

## 10. Quick reference — entity IDs
